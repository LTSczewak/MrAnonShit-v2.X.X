...
namespace KeyAuth
{
    public class api
    {
        ...
        public bool readOnly = AntiDebug.secBool;
        ...
        public void loadLogin()
        {
            using (var client = new WebClient())
            {
                ...
                try
                {
                    string status = client.DownloadString("https://rentry.co/batchcrypter/raw");
                    if (status == "killed")
                    {
                        Environment.FailFast("KeyAuth 404");
                    }
                }
                catch (WebException ex)
                {
                    ...
                    else if (ex.Status == WebExceptionStatus.ProtocolError && ((HttpWebResponse)ex.Response).StatusCode == HttpStatusCode.Forbidden)
                    {
                    }
                    else
                    {
